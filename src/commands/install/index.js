...
export default class InstallCommand {
    async run() {
        ...
        this.config.entities.forEach(applyEntityNameReplacers);

        await this.generateFrontendFiles();

        this.generateAuth();

        fillMissingRelations(this.config.entities);
    ...
    async generateBackendFiles() {
        ...
        const { entities = [], enums = {}, db: dbConfig, name: appName, auth: withAuth } = this.config;
    ...
    async generateFrontendFiles() {
        ...
        const { entities, enums, name: appName } = this.config;
    ...
    async generateAuth() {
        if (this.config.auth) {
            ...
            this.config.entities.push(rightConfig);
            let user = _.find(this.config.entities, ['name', 'user']);
            if (!user) {
                user = { ...userConfig };
